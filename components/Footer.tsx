      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="flex flex-col items-center gap-8 text-center sm:flex-row sm:items-start sm:justify-between sm:text-left">
          ...
          <div className="max-w-xs">
            ...
            <p className="mt-2 text-[13px] text-cream/45 leading-relaxed">
          ...
          <nav className="flex flex-wrap justify-center gap-x-6 gap-y-2 sm:justify-start">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-[13px] text-cream/55 transition-colors hover:text-cream"
              >
                {link.label}
              </a>
            ))}
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-cream/10 pt-6 sm:flex-row">
          ...
          <button
            onClick={scrollToTop}
            className="flex h-9 w-9 items-center justify-center rounded-full border border-cream/15 text-cream/35 transition-colors hover:border-accent hover:text-accent cursor-pointer"
